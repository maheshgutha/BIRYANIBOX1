    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:520px;margin:auto;background:#111;padding:40px;border-radius:20px;color:#fff;">
          ...
          <p style="color:#888;font-size:12px;margin-top:16px;">Use this code at checkout or add it to your profile on Biryani Box. Valid for 1 year.</p>
...
// ── GET /api/gift-cards/my  — cards received by (or claimed by) logged-in customer ──
router.get('/my', protect, async (req, res, next) => {
  try {
    const cards = await GiftCard.find({
      $or: [
        { receiver_email: req.user.email },
        { claimed_by_user_id: req.user._id },
      ],
      status: 'active',
    }).sort({ created_at: -1 });
...
// ── GET /api/gift-cards/sent  — cards sent by logged-in customer ───────────
...
router.post('/', async (req, res, next) => {
  try {
    ...
    if (delivery_method === 'email' || receiver_email) {
    ...
    // Try to detect the logged-in user from Authorization header (optional)
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      try {
        ...
        if (u && u.role === 'customer') sent_by_user_id = u._id;
      } catch (_) { /* not logged in or token invalid — ok */ }
    }

    const card = await GiftCard.create({
      code, denomination, balance: denomination,
...
// ── POST /api/gift-cards/claim  — customer adds a gift card by unique code ─
router.post('/claim', protect, async (req, res, next) => {
  try {
    ...
    if (card.claimed_by_user_id && card.claimed_by_user_id.toString() === req.user._id.toString()) {
      return res.json({ success: true, data: card, message: 'Already added to your account' });
    }
    // Claim it
    card.claimed_by_user_id = req.user._id;
    await card.save();
    res.json({ success: true, data: card, message: 'Gift card added to your account!' });
  } catch (err) { next(err); }
});
