...
const POS = ({ user }) => {
  ...
  // Clear delivery fields when switching away from Takeaway
  useEffect(() => {
    if (!isDeliveryOrder) {
      setDeliveryAddress('');
      setDeliveryNotes('');
  ...
  const filteredMenu = activeTab ? menu.filter(i => (i.category || '').toLowerCase() === activeTab.toLowerCase()) : menu;
  const isItemAvailable = (item) => (item.is_available ?? item.available ?? true) && (item.stock == null || item.stock > 0);
  ...
  const total    = rawTotal - discount + deliveryFee;
  ...
  const handlePlaceOrder = () => {
    ...
    if (isDeliveryOrder) {
      ...
      if (!distanceKm || isNaN(d) || d <= 0) {
        setDeliveryError('Please enter the delivery distance in km.');
        return;
      }
      if (d > 10) {
        setDeliveryError(`We only deliver within 10 km. The entered distance (${d} km) exceeds our limit.`);
        return;
      }
    }
          ...
          >
            ...
            >
              <div className="flex items-center gap-4 mb-6">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${isDeliveryOrder ? 'bg-primary/20 text-primary' : 'bg-primary/20 text-primary'}`}>
                  {isDeliveryOrder ? <Truck size={24} /> : <ShoppingCart size={24} />}
                </div>
                <div>
                  <h3 className="text-xl font-black text-white">Confirm Order</h3>
                  <p className="text-xs text-text-muted uppercase tracking-widest">
                    {selectedTable} · {isDeliveryOrder ? `${dist} km delivery` : `${cart.length} item type${cart.length > 1 ? 's' : ''}`}
                  </p>
                </div>
              </div>
              ...
              {/* Delivery info box */}
              {isDeliveryOrder && (
                <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4 mb-4 space-y-2">
              ...
              {isDeliveryOrder && (
                <p className="text-[10px] text-yellow-400/80 text-center mb-4 flex items-center justify-center gap-1.5">
                  <Truck size={11} /> Rider will be assigned once owner marks order as Paid
                </p>
              )}
        ...
        <div className="lg:col-span-3 space-y-6 overflow-y-auto pr-4 scrollbar-hide">
          ...
          <AnimatePresence>
            {isDeliveryOrder && (
              ...
              >
                <div className="bg-primary/5 border border-primary/25 rounded-2xl p-6 space-y-4">
                  {/* Header */}
                  ...
                  {deliveryError && (
                    <div className="flex items-start gap-2 bg-red-500/10 border border-red-500/30 rounded-xl px-4 py-3">
                      ...
                      <p className="text-xs text-red-400 font-bold">{deliveryError}</p>
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <MapPin size={14} className="absolute left-3 top-3.5 text-primary shrink-0" />
                      <textarea
                        value={deliveryAddress}
                        onChange={e => { setDeliveryAddress(e.target.value); setDeliveryError(''); }}
                        placeholder="House No., Street, Area, Landmark, City, PIN…"
                        rows={2}
                        className="w-full bg-bg-main border border-white/10 p-3 pl-8 rounded-xl text-sm text-white placeholder:text-white/20 focus:border-primary outline-none resize-none transition-colors"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        step="0.1"
                        value={distanceKm}
                        onChange={e => { setDistanceKm(e.target.value); setDeliveryError(''); }}
                        placeholder="e.g. 3.5"
                        className={`w-full bg-bg-main border p-3 rounded-xl text-sm text-white placeholder:text-white/20 focus:border-primary outline-none transition-colors ${
                          dist > 10 ? 'border-red-500/50 bg-red-500/5' : 'border-white/10'
                        }`}
                      />
                    </div>
                    <div>
                      ...
                      <div className={`w-full p-3 rounded-xl text-sm font-black border ${
                        dist > 10
                  ...
                  {dist > 10 && (
                    <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/30 rounded-xl px-4 py-3">
                      ...
                      <p className="text-xs text-red-400 font-bold">We only deliver within 10 km. This order cannot be placed.</p>
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-bg-main border border-white/10 p-3 rounded-xl text-sm text-white placeholder:text-white/20 focus:border-primary outline-none transition-colors"
                    />
                  </div>

                  <p className="text-[10px] text-text-muted flex items-center gap-1.5">
                    <Truck size={10} className="text-primary" />
                    Rider will be dispatched when owner marks order as <span className="text-primary font-bold">Paid</span>
                  </p>
                </div>
              </MotionDiv>
            )}
            ...
            <div className="mb-3">
              {couponApplied ? (
                <div className="flex items-center justify-between bg-green-500/10 border border-green-500/20 rounded-xl px-4 py-2.5">
                  <p className="text-xs text-green-400 font-bold">🎉 {couponApplied.code} — {couponApplied.discount}% OFF</p>
                  <button onClick={() => setCouponApplied(null)} className="text-text-muted hover:text-white"><X size={14} /></button>
                </div>
              ) : (
                <div className="flex gap-2">
                  ...
                  <button onClick={() => {
                    ...
                    const knownCoupons = [{ code: 'BB-RX1-C5-VGK3', discount: 20 }];
                    const found = knownCoupons.find(c => c.code === couponInput.trim());
                    if (found) { setCouponApplied(found); setCouponInput(''); }
                    else alert('Invalid or already used coupon.');
          ...
          <div className="pt-4 border-t border-white/10 space-y-3">
            <div className="space-y-1.5">
              <div className="flex justify-between items-center text-xs text-text-muted font-bold uppercase tracking-wider">
                <span>Table</span><span className="text-white">{selectedTable}</span>
              </div>
            ...
            <button
              ...
              disabled={cart.length === 0 || (isDeliveryOrder && dist > 10)}
              className={`w-full py-5 rounded-2xl flex items-center justify-center gap-3 font-black uppercase tracking-widest transition-all shadow-xl text-sm ${
                cart.length === 0 || (isDeliveryOrder && dist > 10)
            ...
            {isDeliveryOrder && (
              <p className="text-[10px] text-text-muted text-center">
                Rider dispatched when owner marks <span className="text-primary font-bold">Paid</span>
              </p>
            )}
          </div>
