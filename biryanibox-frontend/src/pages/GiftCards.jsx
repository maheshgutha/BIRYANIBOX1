    ...
    <div className="min-h-screen bg-bg-main text-white pt-32 pb-20 relative overflow-hidden">
      ...
      <div className="container max-w-5xl mx-auto relative z-10 px-6">
        ...
        <div className="mb-14">
          ...
          <p className="text-lg text-text-muted max-w-xl">Send a unique one-time gift card — the recipient gets the code instantly via email.</p>
        </div>

        {/* One-time use notice */}
        <div className="flex items-center gap-3 bg-amber-500/10 border border-amber-500/20 rounded-2xl px-5 py-3 mb-8 w-fit">
          <AlertCircle size={14} className="text-amber-400 shrink-0" />
          <p className="text-xs font-bold text-amber-300">Gift cards are <strong>one-time use only</strong> — the full amount is applied in a single transaction.</p>
        </div>
                ...
                {myCards.map(card => (
                  <div key={card._id} className="bg-secondary/40 border border-white/10 rounded-3xl p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border ${statusBadge(card.status)}`}>
                        {card.status === 'active' ? '✓ Active' : card.status === 'used' ? '✗ Used' : card.status}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 font-mono text-base font-black text-white/80 tracking-widest bg-white/5 px-4 py-3 rounded-xl mb-3">
                    ...
                    <div className="flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3 py-2 mb-3">
                      ...
                      <p className="text-[10px] font-bold text-amber-300">One-time use · Full ₹{card.denomination} applied at once</p>
                    </div>
                    <p className="text-xs text-white/30">From: {card.sender_name || card.purchased_by || 'Someone'}</p>
                    {card.expiry_date && <p className="text-xs text-white/20 mt-1">Expires: {new Date(card.expiry_date).toLocaleDateString()}</p>}
                  </div>
                ))}
              </div>
            )}
              ...
              <AnimatePresence mode="wait">
                        ...
                        <div className="space-y-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-white/20 mt-1.5 flex items-center gap-1">
                              <Mail size={9} /> The unique one-time code will be emailed here instantly.
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                {step === 3 && (
                  <motion.div key="s3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    ...
                    <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl mb-4 flex items-start gap-3">
                      <AlertCircle size={14} className="text-amber-400 mt-0.5 shrink-0" />
                      <p className="text-xs text-amber-300 font-bold">
                        This gift card is <strong>one-time use only</strong>. The full ₹{amount} will be consumed in a single redemption.
                      </p>
                    </div>
                ...
                {step === 4 && (
                  <motion.div key="s4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center">
                    ...
                    <div className="flex items-center gap-2 justify-center bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-2 mb-6">
                      <AlertCircle size={12} className="text-amber-400" />
                      <p className="text-[10px] font-bold text-amber-300">One-time use — expires after first redemption</p>
                    </div>
