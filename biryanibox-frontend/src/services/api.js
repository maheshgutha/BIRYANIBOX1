...
export const ordersAPI = {
  ...
  confirmOrder:  (id, action)        => request(`/orders/${id}/confirm`, { method: 'PATCH', body: JSON.stringify({ action }) }),
...
export const reservationsAPI = {
  ...
  publicSearch: (name, phone, email) =>
    request(`/reservations/public/search?name=${encodeURIComponent(name || '')}&phone=${encodeURIComponent(phone || '')}&email=${encodeURIComponent(email || '')}`),
...
// ── CATERING ──────────────────────────────────────────────────────
// Single export — includes both admin ops and the 1-day reminder trigger
export const cateringAPI = {
  ...
  // Sends notifications + emails to owner/manager/customer for events happening tomorrow
  sendReminders: ()           => request('/catering/send-reminders'),
};
...
export const giftCardsAPI = {
  ...
  // body: { code, amount_to_use? (partial use — balance carries over), order_id? }
...
export const deliveryAPI = {
  ...
  getMySkipped:   ()            => request('/deliveries/my-skipped'),
  getLiveLocations: ()          => request('/deliveries/live-locations'),
  updateLocation: (id, lat, lng, address) => request(`/deliveries/${id}/location`, { method: 'PATCH', body: JSON.stringify({ lat, lng, address }) }),
